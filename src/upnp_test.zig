...
pub fn main() !void {
    ...
    // TCP 포트 매핑
    upnp_client.addPortMapping(port, port, upnp.UPnPClient.PortMapping.Protocol.TCP, "Eastsea P2P Node TCP", 3600 // 1시간 임대
    ) catch |err| {
        print("⚠️ Failed to add TCP port mapping: {any}\n", .{err});
    };

    // UDP 포트 매핑 (DHT용)
    upnp_client.addPortMapping(port, port, upnp.UPnPClient.PortMapping.Protocol.UDP, "Eastsea DHT Node UDP", 3600 // 1시간 임대
    ) catch |err| {
        ...
        if (try stdin.readUntilDelimiterOrEof(input_buffer[0..], '\n')) |input| {
            ...
            if (std.mem.eql(u8, trimmed, "q") or std.mem.eql(u8, trimmed, "quit")) {
                ...
                if (try stdin.readUntilDelimiterOrEof(input_buffer[0..], '\n')) |port_input| {
                    ...
                    upnp_client.addPortMapping(new_port, new_port, upnp.UPnPClient.PortMapping.Protocol.TCP, "Eastsea Test Port", 1800 // 30분 임대
                    ) catch |err| {
                ...
                if (try stdin.readUntilDelimiterOrEof(input_buffer[0..], '\n')) |port_input| {
                    ...
                    upnp_client.removePortMapping(remove_port, upnp.UPnPClient.PortMapping.Protocol.TCP) catch |err| {
